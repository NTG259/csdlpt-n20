...
- [ ] `src/lib/main-api-client.ts` — `mainApiClient.get/post`.
- [ ] `src/lib/regional-api-client.ts` — `getRegionalApiClient` map BAC→8081, NAM→8082.
- [ ] Gọi thử `mainApiClient.get(MAIN_ENDPOINTS.REGIONS)` → trả mảng `[{maKhuVuc:"Bac"...},{maKhuVuc:"Nam"...}]`.
- [ ] Xác nhận env URL đọc được ở client (`NEXT_PUBLIC_*`).
