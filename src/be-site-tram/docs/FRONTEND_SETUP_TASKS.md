...
Project FE hiện nằm ở thư mục **`sitemain-fe/`** (cạnh `sitemain/`) và đã hoàn thành **Phase 1, 2, 3**.
...
| 6 | API clients | `mainApiClient` + `regionalApiClient` | ⬜ Chưa làm |
| 7 | Auth state | Lưu token/user, context, chọn vùng theo user | ⬜ Chưa làm |
| 8 | Layout & Providers | QueryProvider, Header, Toaster, route layout | ⬜ Chưa làm |
| 9 | Feature: Auth | `/login`, `/register` | ⬜ Chưa làm |
| 10 | Feature: Sản phẩm | `/products`, `/products/[maSP]`, trang `/` | ⬜ Chưa làm |
| 11 | Feature: Giỏ hàng | `/cart` (gọi API theo vùng) | ⬜ Chưa làm |
...
## Task 10.3 — Trang `/products` (danh sách + lọc + phân trang)
- **Mục tiêu:** Lưới sản phẩm (Card), bộ lọc danh mục/thương hiệu, nút phân trang.
- **File tạo:** `src/app/products/page.tsx`, `src/features/products/product-card.tsx`, `product-grid.tsx`, `product-filters.tsx`, `pagination.tsx`.
- **Kết quả mong muốn:** Xem được sản phẩm, lọc + chuyển trang chạy mượt, ảnh + giá hiển thị đúng.
...
> ⚠️ Endpoint chi tiết của giỏ hàng nằm ở **API Site Bắc/Nam** (chưa có trong tài liệu Site Main). Dùng path **dự kiến** `/api/cart`, chỉnh lại khi có doc chính thức. Mọi lời gọi đi qua `getRegionalApiClient(region)`.
...
- **Kết quả mong muốn:** User Bắc thao tác giỏ trên `:8081`, user Nam trên `:8082` (kiểm chứng ở tab Network).

## Task 11.2 — Trang `/cart`
- **Mục tiêu:** Danh sách item, tăng/giảm số lượng, xoá, tổng tiền, nút **Đặt hàng**.
- **File tạo:** `src/app/cart/page.tsx`, `src/features/cart/cart-item-row.tsx`, `cart-summary.tsx`.
- **Kết quả mong muốn:** Giỏ phản ánh đúng dữ liệu vùng; đổi số lượng cập nhật tổng tiền; chưa đăng nhập → chuyển `/login`.
...
> **Ghi chú:** Các endpoint giỏ hàng/đơn hàng (`/api/cart`, `/api/orders`) là **dự kiến** vì tài liệu hiện chỉ mô tả Site Main. Khi có doc Site Bắc/Nam, chỉ cần cập nhật path trong `constants/endpoints.ts` và type trong `types/domain.ts` — phần định tuyến theo vùng đã sẵn sàng.
