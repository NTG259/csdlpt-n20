...
- [ ] `schemas.ts` khớp ràng buộc BE (phone regex, mật khẩu 6–72, cccd 12 số).
- [ ] `use-regions.ts` đổ dropdown từ `/api/regions`.
- [ ] `use-auth-mutations.ts` — login & register, register tự đăng nhập.
- [ ] `/login` và `/register` chạy, lỗi hiển thị `message` Việt hoá.
