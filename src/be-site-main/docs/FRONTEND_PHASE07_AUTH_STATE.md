...
- Token/session sống 1 năm (`expiresIn = 31536000` giây từ backend). Có thể lưu kèm thời điểm hết hạn để chủ động ẩn UI khi quá hạn.
...
- [x] `src/lib/auth-storage.ts` — `saveSession/getSession/clearSession`, có `expiresAt` + guard SSR.
- [x] `src/features/auth/region.ts` — `getUserRegion` (Bac→BAC, Nam→NAM).
- [x] `src/features/auth/auth-context.tsx` — `AuthProvider`, `useAuth`, nối `setUnauthorizedHandler`.
- [ ] Test tay: gán session giả → reload vẫn còn; gọi `logout()` → về `/login`.

> Ghi chú verify: helper `auth-storage` và `getUserRegion` đã được probe bằng mock `localStorage`; phần redirect thật của `logout()` chưa test tay trong UI vì `AuthProvider` sẽ được gắn vào app ở Phase 8.
