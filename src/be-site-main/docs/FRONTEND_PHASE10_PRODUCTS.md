...
> **Mục tiêu phase:** Xem sản phẩm mượt, tìm kiếm text, lọc theo danh mục/thương hiệu/trạng thái, sắp xếp và phân trang đúng cấu trúc `PageResponse`.
...
> **Ghi chú triển khai:** Backend `GET /api/products` hiện chưa có query `q`/keyword. FE vẫn hỗ trợ ô tìm kiếm và đồng bộ `q` trên URL, nhưng `q` được lọc frontend trên `items` của trang hiện tại. Muốn tìm toàn bộ catalog theo keyword cần bổ sung tham số search ở backend.
...
## Task 10.4 — Trang `/products` (danh sách + tìm kiếm + lọc + phân trang)

- **Mục tiêu:** Lưới Card + tìm kiếm + bộ lọc + phân trang.
- **File tạo:**
  - `src/app/products/page.tsx` — route server bọc `Suspense`.
  - `src/features/products/product-list-page.tsx` — đọc filter/page từ `useSearchParams`, gọi `useProducts`.
  ...
  - `src/features/products/product-filters.tsx` — search input, `Select` danh mục/thương hiệu/trạng thái/sort/size.
  - `src/features/products/pagination.tsx` — Trước/Sau dựa vào `page`, `last`, `totalPages`.
- **Lưu ý:** giữ filter/page trên URL (`?q=&page=&maDanhMuc=&maThuongHieu=&trangThai=&sort=&size=`) để chia sẻ link & back/forward đúng.
...
- **File chỉnh/tạo:** `src/app/page.tsx`, `src/features/products/home-products.tsx` — banner + vài sản phẩm mới (`useProducts({ size: 8 })`) + CTA "Xem tất cả" → `/products`.
...
- [x] `use-products.ts` — `useProducts` (query + placeholderData), `useProduct`.
- [x] `use-reference.ts` — `useCategories`, `useBrands`.
- [x] `format.ts` — `formatVnd`, `productImageUrl`.
- [x] `/products` — grid + search/filter/sort/size/pagination, đồng bộ URL.
- [x] `/products/[maSP]` — chi tiết + xử lý 404 + nút thêm giỏ ở trạng thái chờ Phase 11.
- [x] `/` — landing + sản phẩm mới.
- [x] Build checks: `bunx tsc --noEmit`, `bun run lint`, `bun run build`.
- [ ] Network/E2E: mọi request sản phẩm/danh mục/thương hiệu trỏ `:8080` khi backend Main và SQL Server chạy được.
