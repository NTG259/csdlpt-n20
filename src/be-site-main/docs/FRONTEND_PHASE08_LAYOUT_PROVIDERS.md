...
- [x] `src/lib/query-client.ts` — `makeQueryClient` (retry thông minh).
- [x] `src/app/providers.tsx` — Query + Auth + Toaster + Devtools.
- [x] `src/app/layout.tsx` — bọc `<Providers>` + render `<Header/>`.
- [x] `src/components/shared/` — `header`, `page-container`, `loading`, `empty-state`.
- [x] Chạy `bun dev`: Header hiển thị, toast thử nghiệm chạy, Devtools mở được.

> Ghi chú verify: `bun dev` đã chạy ở `http://localhost:3000` và trang chủ trả `200`. `Toaster` và React Query Devtools đã được mount trong `Providers`; thao tác toast thực tế sẽ có điểm trigger rõ hơn từ Phase 9 trở đi.
